...
const featureFlags: ArchitectureDecision = {
  slug: 'feature-flags',
  title: 'Feature flags — tier-gated, cache-aside, zero vendor',
  subtitle:
    'A database table and a Redis cache replace a SaaS subscription. The middleware enforces it before business logic runs.',
  badge: 'Infrastructure',
  context:
    'The application supports three user tiers — DEMO, TRIAL, and FULL — each with different feature access. DEMO users can browse but not mutate. TRIAL users get full access for a limited time. FULL users get everything. The naive approach is conditional checks scattered across handlers and components. The SaaS approach (LaunchDarkly, Unleash) adds vendor lock-in, runtime dependencies, and cost for a problem that is fundamentally a database lookup. The application needed feature gating that is enforceable, cacheable, and owned entirely by the codebase.',
  decision:
  ...
  rationale: [
    ...
    'Middleware enforcement means feature gating cannot be forgotten. It runs in the action chain before input validation and before the handler. A developer cannot accidentally expose a gated feature — the middleware throws FeatureDisabledException before the action function is reached.',
  ...
  tradeoffs: [
    {
      ...
      con: 'No runtime toggle UI. Flags are currently seed-only — changing a flag requires a database update or re-seed, not a dashboard click. An admin UI is planned but not yet built.',
    },
    {
    ...
    {
      pro: 'Cache-aside with one-hour TTL keeps feature checks sub-millisecond for the common case. No cold-start penalty — cache populates on first access.',
      con: 'Flag changes are not instant. A disabled feature remains cached as enabled for up to one hour. For emergency disabling, the cache must be manually invalidated or the TTL reduced. Not acceptable for kill-switch scenarios at scale.',
    },
    {
      ...
      con: 'No user-level targeting or gradual rollouts. If a feature needs to be enabled for one specific user or rolled out to 10% of a tier, the current model cannot express that. This would require a schema change.',
    },
  ],
  codeBlocks: [
    {
