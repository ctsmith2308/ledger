...
const mfa: ArchitectureDecision = {
  ...
  rationale: [
    'TOTP is an open standard (RFC 6238) supported by every major authenticator app — Google Authenticator, Authy, 1Password. No vendor dependency, no SMS costs, no phone number required.',
    'The two-step login uses a purpose-based challenge token (5-minute TTL, purpose: mfa_challenge) signed by the same JwtService. No database state, no challenge table — the token itself proves the password was verified.',
  ...
  tradeoffs: [
    ...
    },
    {
      ...
      con: 'The secret is stored in plaintext. A database breach exposes TOTP seeds. Production hardening: encrypt at rest via application-level encryption or database column encryption.',
    },
    {
...
if (user.mfaEnabled) {
  return Result.ok({ type: 'MFA_REQUIRED', user });
}
...
// IdentityService — signs based on the union
...
const token = await this.jwtService.sign(userId, purpose, ttl);`,
    },
    {
...
await this.eventBus.dispatch(events);`,
  ...
  disableMfa(): void {
    ...
    },
    {
      ...
      code: `// Login action — set cookie on success, store challenge on MFA
